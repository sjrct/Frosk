...
meta * new_meta(node * branch)
{
	uint hdr_size;
	node * fr = malloc(sizeof(node));
	...
	if (branch != NULL) {
		...
		hdr_size = align(sizeof(fmeta_header), ENTRY_SIZE);
		m->shift = 0;
...
static node * find_free(uint size)
{
	node * f = fs.free_meta;
...
static node * new_nodeh(uint size, int nometa)
{
	node * new, * p;
	node * fnd;
	...
	if (fnd == NULL) {
		puts("no free node found.");
		
		if (nometa) return NULL;
	
		// create a new meta block
		fnd = new_nodeh(sizeof(fnode_branch), 1);
		if (fnd == NULL) {
			assert(0);
		}
...
node * new_node(uint size)
{
	return new_nodeh(size, 0);
}

static node * new_common(const char * name, node * parent)
{
	node * new = new_node(sizeof(fnode_common));
...
node * new_file(const char * name, const char * actual, node * parent)
{
	qword size;
	FILE * tmp;
	node * new, * bl;
	...
	bl = new_node(sizeof(fnode_block));
	bl->type = FNODE_BLOCK;
...
node * new_dir(const char * name, node * parent)
{
	node * new = new_common(name, parent);
	new->type = FNODE_DIRECTORY;
	new->u.common.data = NULL;
	return new;
}

node * new_indir(const char * name, const char * to, node * parent)
{
	node * new = new_common(name, parent);
...
node * get_string(const char * str)
{
	int cmp, length;
	node * n;
	node ** m = &fs.strtbl;
	...
	n = new_node(sizeof(fnode_string) + length);
	n->type = FNODE_STRING;
...
static void debug_node(FILE * f, node * n)
{
	if (n == NULL) return;
...
static void debug_meta(FILE * f, meta * m)
{
	if (m == NULL) return;
...
static void seek_node(FILE * f, node * n, qword bl)
{
...
static void write_meta(FILE *, meta *);

static void write_node(FILE * f, node * n)
{
	FILE * g;
	fnode wn;
	qword i, old;
	...
	for (bl = 0; bl <= 1; bl++) {
		...
		switch (n->type) {
		case FNODE_FREE:
			fwrite(&wn, 1, sizeof(fnode_free) + FNODE_ADD_SIZE, f);
			break;
		
		case FNODE_STRING:
			fwrite(&wn, 1, sizeof(fnode_string) + FNODE_ADD_SIZE, f);
...
static void write_meta(FILE * f, meta * m)
{
	if (m == NULL) return;	
...
void write_fs(FILE * f)
{
	...
	fwrite((char *)&hdr, 1, sizeof(fmeta_header), f);
